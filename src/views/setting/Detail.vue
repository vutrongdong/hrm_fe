        ...
        <v-list three-line>
          <template v-for="(item, index) in items.data">
            <div style="padding-left:30px">
              <p style="text-transform:capitalize;width:100%">{{ item.name }} : {{ item.value }}
                <span>
                 <v-btn v-if="canAccess('setting.update')" icon @click="$router.push({name: 'setting-edit', params: {id: $route.params.id}})">
  ...
  export default{
    ...
    methods: {
      ...
      ...mapActions('Setting',['getSetting', 'deleteSetting']),
