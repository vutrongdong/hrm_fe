<template>
  <div>
    <div class="image-container position-relative text-center" v-if="!images.length">
      <div class="drag-upload-cover position-absolute" v-if="isDragover" @drop="onDrop">
        <div class="centered full-width text-center text-primary">
          ...
          <h4><b>Drop your files here...</b></h4>
        </div>
      </div>
      <div v-else class="image-center position-absolute display-flex flex-column justify-content-center align-items-center" @dragover.prevent="onDragover" @drop="onDrop">
        <div>
    ...
    <div class="image-container position-relative text-center image-list" v-else>
      <div class="preview-image full-width position-relative cursor-pointer" @click="dialogImage = true">
      ...
      <div class="image-bottom display-flex position-absolute full-width justify-content-between align-items-center">
        <div class="image-bottom-left display-flex align-items-center">
          <div class="display-flex align-items-center" v-show="imageDefault">
            ...
            <v-tooltip top>
              <i slot="activator" class="cursor-pointer display-flex align-items-center">
          ...
          <a class="text-small" @click.prevent="markIsPrimary(currentIndexImage)" v-show="!imageDefault">{{markIsPrimaryText}}</a>
        </div>
        <a class="image-delete display-flex" @click.prevent="deleteImage(currentIndexImage)">
    ...
    <div class="image-list-container display-flex" v-if="images.length && multiple">
      ...
      <div class="image-list-item position-relative cursor-pointer display-flex justify-content-center align-items-center" v-if="images.length < 5">
    ...
    <v-dialog v-model="dialogImage" width="840px" height="600px">
      <v-card>
        <v-carousel v-if="images.length" class="custom-carousel" :value="currentIndexImage" @input="changeHighlight" :interval="100000">
          <v-carousel-item
...
<script>
import { forEach, findIndex, orderBy } from 'lodash'
export default {
  ...
  props: {
    ...
    markIsPrimaryText: {
      type: String,
      default: 'Mark is primary'
    },
    popupText: {
      type: String,
      default: 'Hình ảnh này sẽ được hiển thị trong tất cả các giao dịch của bạn cho mặt hàng này'
    },
    accept: {
      type: String,
  ...
  computed: {
    imagePreview () {
      // let index = this.images.findIndex(x => x.highlight === 1)
  ...
  methods: {
    ...
    markIsPrimary (currentIndex) {
      ...
      this.$emit('data-change', this.images)
      this.$emit('mark-is-primary', this.images)
    },
    deleteImage (currentIndex) {
      ...
      this.$emit('data-change', this.images)
      this.$emit('delete-image', this.images)
    }
  },
  watch: {
    dataImages: {
...
.custom-carousel .v-carousel__prev .v-btn .v-btn__content .v-icon, .custom-carousel .v-carousel__next .v-btn .v-btn__content .v-icon {
  color: #000;
}
.drag-upload-cover {
  top: 0;
